...
import { db, storage } from "../../lib/firebase"; // Ensure storage is configured
            ...
            <div className="center">
                {chat?.messages?.map((message, index) => (
                    <div className={`message own`} key={index}>
                        {message.img && <img src={message.img} alt="Message Image" className="message-img" />}
                        <div className="texts">
                            <p>{message.text}</p>
                            <span>1 min ago</span> {/* Replace with actual timestamp logic */}
                        </div>
                    </div>
                ))}
                {img.url && (
